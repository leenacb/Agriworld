...
<body class="far">
	...
	<div class="main-w3layouts wrapper">
		<a href="farmer_auction_table.php" class="btn btn-outline-primary">< previous</a>
		<h1> Auction Apply Form</h1>
...
<!-- copyright -->
		...
		<ul class="colorlib-bubbles"></ul>
				<form action="auction_action.php" method="post">
				<?php
				$link=mysqli_connect("localhost","root","","agriworld");
				$aid=$_GET['id'];
				echo $aid;
				$au="select * from tb_auction where id=$aid";
				$rlt=mysqli_query($link, $au);
				while($row=mysqli_fetch_assoc($rlt))
   {
				?>
					
					<input class="text" type="text" name="productname" value="<?php echo $row['crop_name']?>" placeholder="product" required=""></br>
					...
					<input class="text" type="text" name="tot" placeholder="Total price" required=""></br>
...
<thead>
   <tr>
       ...
       <th class="bg-success">Farmer name</th>
...
$sql="select * from tb_auction_apply where ";
